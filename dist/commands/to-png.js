...
export async function toPngCommand(file, options) {
    const spinner = ora("Converting PDF to PNG...").start();
    try {
        ...
        for (const pageNum of pagesToConvert) {
            spinner.text = `Converting page ${pageNum}/${totalPages} to PNG...`;
            ...
            // Save as PNG
            const outputFile = path.join(dirName, `${baseName}-page-${pageNum}.png`);
            const buffer = canvas.toBuffer("image/png");
            fs.writeFileSync(outputFile, buffer);
        }
        spinner.succeed(chalk.green("PDF converted to PNG successfully!"));
        console.log(chalk.cyan(`Pages converted: ${pagesToConvert.length}`));
        console.log(chalk.bold(`Output pattern: ${baseName}-page-{page}.png`));
    }
    catch (error) {
        spinner.fail(chalk.red("Failed to convert PDF to PNG"));
        console.error(chalk.red(error instanceof Error ? error.message : "Unknown error"));
        process.exit(1);
    }
}
//# sourceMappingURL=to-png.js.map